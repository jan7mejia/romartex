...
<div class="max-w-4xl mx-auto bg-slate-900 p-8 rounded-3xl border border-slate-700 shadow-2xl">
    ...
    <form action="{{ route('admin.repuestos.store', $tipo) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-800/40 p-6 rounded-2xl border border-slate-700">
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Imagen del Producto</label>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="codigo" required class="w-full bg-slate-900 border border-slate-700 rounded-xl p-2 text-white">
                </div>
                <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="0.01" name="precio" required class="w-full bg-slate-900 border border-slate-700 rounded-xl p-2 text-green-400 font-mono">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase">Descripción / Observaciones</label>
        ...
        <div class="bg-slate-800/60 p-6 rounded-2xl border border-slate-700">
            <h3 class="text-xs font-black text-slate-400 uppercase mb-4 tracking-widest">Especificaciones Técnicas del {{ $tipo }}</h3>
            ...
            @if($tipo == 'bendix')
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div><label class="text-xs text-slate-500 uppercase">Ø Externo</label><input type="text" name="diametro_externo" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white"></div>
                    <div><label class="text-xs text-slate-500 uppercase">Ø Interno</label><input type="text" name="diametro_interno" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white"></div>
                    <div><label class="text-xs text-slate-500 uppercase">Largo</label><input type="text" name="largo" required class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white"></div>
                </div>
            @elseif($tipo == 'inducido')
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div><label class="text-xs text-slate-500 uppercase">Largo (mm)</label><input type="text" name="largo" required class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white"></div>
                    <div><label class="text-xs text-slate-500 uppercase">Diámetro</label><input type="text" name="diametro" required class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white"></div>
                    <div><label class="text-xs text-slate-500 uppercase">Estrías</label><input type="number" name="estrias" required class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white"></div>
                    ...
                    <div><label class="text-xs text-slate-500 uppercase">Código Original</label><input type="text" name="codigo_original" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white"></div>
                </div>
        ...
        <div class="flex flex-col md:flex-row gap-4">
            <button type="submit" class="flex-[2] bg-green-500 text-black font-black py-4 rounded-2xl uppercase hover:bg-green-400 transition-all shadow-lg">
                Registrar {{ ucfirst($tipo) }} en Base de Datos
            </button>
